...
高度兼容PostgreSQL、Cassandra、Oracle和MYSQL，覆盖绝大多数常见功能，提供自动迁移工具，支持迁移评估，以保障数据迁移安全。
...
构建以DRAM+NVRAM内存池为中心的存算一体化体系架构，使CPU、GPU、FPGA及xPU等异构处理器共享内存池，在内存内实现数据存储与计算的统一，实现自适应异构处理器的异构智能计算引擎运行环境，有效防止异构处理器缓存数据频繁迁移而导致的井喷效应，支撑异构处理器的协同并行计算，突破冯·诺依曼体系架构固有的性能瓶颈。
